...
DATAPATH = "G:/Desktop/Desktop_NEU/Cursor_Project/XML-Dateien/"
...
def export_eventtypes_to_csv(eventtypes, filename="eventtypes_export.csv"):
    fields = ["Address", "FCRead", "Conversion", "Parameter", "BlockLength", "BytePosition", "ByteLength", "BitPosition", "BitLength", "LowerBorder", "UpperBorder", "Stepping", "ID"]
    with open(filename, mode="w", encoding="utf-8", newline="") as csvfile:
        ...
        for event in eventtypes:
            ...
            writer.writerow(row)
    print(f"EventTypes wurden als CSV gespeichert: {filename}")


def export_eventtypes_to_xml(eventtypes, filename="eventtypes_export.xml"):
    from xml.etree.ElementTree import Element, SubElement, ElementTree

    root = Element('EventTypes')
    for event in eventtypes:
        event_elem = SubElement(root, "EventType")
        for key in ["Address", "FCRead", "Conversion", "Parameter", "BlockLength", "BytePosition", "ByteLength", "BitPosition", "BitLength", "LowerBorder", "UpperBorder", "Stepping", "ID"]:
            SubElement(event_elem, key).text = str(event.get(key, ''))
    tree = ElementTree(root)
    tree.write(filename, encoding="utf-8", xml_declaration=True)
    print(f"EventTypes wurden als XML gespeichert: {filename}")




if __name__ == "__main__":
    parse_Textresource('de')  # oder 'en'
    eventtypes = parse_ecnEventType()
    export_eventtypes_to_csv(eventtypes)
    export_eventtypes_to_xml(eventtypes)
